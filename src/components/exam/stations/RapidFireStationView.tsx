...
/**
 * Speaking Station 2 — Rapid-Fire (Part 1 audio examiner).
 *
 * 3 Part 1 questions, one at a time. Examiner audio auto-plays, then a
 * "Start Speaking" button appears. `answerSeconds` (30) to answer, smart
 * cut-off via Lock & Next. Audio captured by MediaRecorder, uploaded in the
 * background, transcribed by Whisper on submit.
 */
export function RapidFireStationView({
  station,
  onComplete,
}: {
  station: RapidFireStation;
  ...
  const firstUnanswered = station.questions.findIndex((q) => {
    ...
    return rec?.payload?.kind !== "rapidFire" || !rec.payload.audioPath;
  });
    ...
    <section>
      ...
      <div className="space-y-4">
        {station.questions.map((q, qi) => {
          ...
          const audioPath =
            rec?.payload?.kind === "rapidFire" ? rec.payload.audioPath : "";
          ...
          return (
            ...
            >
              ...
              {isActive ? (
                <RapidFireRecorder stationId={station.id} question={q} />
              ) : isDone ? (
...
function RapidFireRecorder({
  stationId,
  question,
}: {
  stationId: string;
  question: RapidFireQuestion;
}) {
  ...
  const { state, start, stop } = useMediaRecorder({
    ...
    onAudioStop: (blob, mimeType) => {
      void enqueueSpeakingUpload({
        ...
        .then((audioPath) => {
          setAudioPath(stationId, question.id, audioPath, "rapidFire");
        })
        .catch((err) => {
          console.error(
            `[rapidFire] background upload failed for ${question.id}:`,
            err
          );
        });
    },
  ...
  // Two phases: "audio" (examiner speaking) → "speak" (mic live). The moment
  // the examiner audio finishes, the mic auto-opens — no manual button. If the
  // mic grant fails we fall back to a "ready" view so the student isn't stuck.
  const phase: "audio" | "speak" | "ready" = state.recording
    ? "speak"
    : audioDone && state.error
    ? "ready" // mic failed to open — manual retry surface
    : audioDone
    ? "audio" // brief window between audio end and mic grant landing
    : "audio";
  ...
  const lockAndNext = () => {
    ...
    lockAudio(stationId, question.id, "rapidFire");
  };
